...
extern void reduction_Kernel_Wrapper_6_3(dim3 gridSize, dim3 blockSize, int numElements, float* dataIn, float* dataOut);
extern void reduction_Kernel_Wrapper_6_4(dim3 gridSize, dim3 blockSize, int numElements, float* dataIn, float* dataOut);
extern void reduction_Kernel_Wrapper_6_5(dim3 gridSize, dim3 blockSize, int numElements, float* dataIn, float* dataOut);
...
main(int argc, char * argv[])
{
	...
	if (!pinnedMemory)
	{
		...
		h_dataOutIntermed = static_cast<float*>
				(malloc(static_cast<size_t>(gridSize * sizeof(*h_dataOutIntermed))));
	...
	else
	{
		...
		cudaMallocHost(&h_dataOutIntermed,
				static_cast<size_t>(gridSize * sizeof(*h_dataOutIntermed)));
	...
	*h_dataOut = 0;
	...
	cudaMemcpy(d_dataOutIntermed, h_dataOutIntermed, 
			static_cast<size_t>(gridSize * sizeof(*d_dataOutIntermed)), 
			cudaMemcpyHostToDevice);
	...
	if(chCommandLineGetBool("gpu-red-initial", argc, argv))
	{
		reduction_Kernel_Wrapper_6_3(grid_dim, block_dim, numElements, d_dataIn, d_dataOutIntermed);
		//The second iteration has one thread block with as many threads as the first iteration had blocks
		reduction_Kernel_Wrapper_6_3(dim3(1), grid_dim, gridSize, d_dataOutIntermed, d_dataOut);
	}
	else if(chCommandLineGetBool("gpu-red-optimized", argc, argv))
	{
		reduction_Kernel_Wrapper_6_4(grid_dim, block_dim, numElements, d_dataIn, d_dataOutIntermed);
		//The second iteration has one thread block with as many threads as the first iteration had blocks
		reduction_Kernel_Wrapper_6_4(dim3(1), grid_dim, gridSize, d_dataOutIntermed, d_dataOut);
	}
	else if(chCommandLineGetBool("gpu-red-optimized-volta", argc, argv))
	{
		reduction_Kernel_Wrapper_6_5(grid_dim, block_dim, numElements, d_dataIn, d_dataOutIntermed);
		//The second iteration has one thread block with as many threads as the first iteration had blocks
		reduction_Kernel_Wrapper_6_5(dim3(1), grid_dim, gridSize, d_dataOutIntermed, d_dataOut);
	}
	else if(chCommandLineGetBool("thrust", argc, argv))
	{
	...
	/*std::cout << "***" << std::endl
			  ...
			  << "***    Time for Reduction: " << 1e3 * kernelTimer.getTime()
				  << " ms" << std::endl
			  << "***    Bandwidth: "
			  	  << 1e-9 * kernelTimer.getBandwidth(numElements)
				  << " GB/s" << std::endl
			  << "***" << std::endl;*/
			  

	std::cout <<  "6: Num Elements: " << numElements << ", blockSize: " << blockSize 
				<< ", Time for Reduction: " << 1e3 * kernelTimer.getTime() << " ms, " 
				<< "Bandwidth: " << 1e-9 * kernelTimer.getBandwidth(numElements) << " GB/s" << std::endl;
	...
	if (!pinnedMemory)
	{
		free(h_dataIn);
		free(h_dataOut);
		free(h_dataOutIntermed);
	}
	else
	{
		cudaFreeHost(h_dataIn);
		cudaFreeHost(h_dataOut);
		cudaFreeHost(h_dataOutIntermed);
	}
	cudaFree(d_dataIn);
	cudaFree(d_dataOut);
	cudaFree(d_dataOutIntermed);
