...
// Kernel Wrappers
//
extern void globalMem2SharedMem_Wrapper(dim3 gridSize, dim3 blockSize, int shmSize, float* gMem);
extern void SharedMem2globalMem_Wrapper(dim3 gridSize, dim3 blockSize, int shmSize, float* gMem);
extern void SharedMem2Registers_Wrapper(dim3 gridSize, dim3 blockSize, int shmSize, float* outFloat);
extern void Registers2SharedMem_Wrapper(dim3 gridSize, dim3 blockSize, int shmSize);
...
main ( int argc, char * argv[] )
{
	...
	for ( int i = 0; i < optNumIterations; i++ )
	{
		...
		{
			globalMem2SharedMem_Wrapper( grid_dim, block_dim, optMemorySize /*Shared Memory Size*/, d_memoryA);
		}
		else if ( chCommandLineGetBool ( "shared2global", argc, argv ) )
		{
			SharedMem2globalMem_Wrapper( grid_dim, block_dim, optMemorySize /*Shared Memory Size*/, d_memoryA);
		}
		else if ( chCommandLineGetBool ( "shared2register", argc, argv ) )
		{
			SharedMem2Registers_Wrapper( grid_dim, block_dim, optMemorySize /*Shared Memory Size*/, outFloat);
		}
		else if ( chCommandLineGetBool ( "register2shared", argc, argv ) )
		{
			Registers2SharedMem_Wrapper( grid_dim, block_dim, optMemorySize /*Shared Memory Size*/);
		}
