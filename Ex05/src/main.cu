...
matMul_Kernel(int matrixSize, float* matrixA, float* matrixB, float* matrixC)
{
    ...
    if (elementIdx < matrixSize && 
        ...
        for (int i = 0; i < matrixSize; i++) {
            result += matrixA[elementIdy * matrixSize + i] * matrixB[i * matrixSize + elementIdx];
        }

        matrixC[elementId] = result;
    }
}
...
main(int argc, char * argv[])
{
    ...
    //
    // Measure CPU version speedup
    //

    ChTimer cputimer;
    if (chCommandLineGetBool("speedup", argc, argv)) {

        cputimer.start();
        MatrixMulOnHost(h_matrixA, h_matrixB, h_matrixC, matrixWidth);
        cputimer.stop();
    }
    ...
    if (chCommandLineGetBool("speedup", argc, argv)) {
        std::cout << "*** Speedup without Data Movement: " << cputimer.getTime() / kernelTimer.getTime() << std::endl;
        std::cout << "*** Speedup with Data Movement: " << cputimer.getTime() / (kernelTimer.getTime() + memCpyH2DTimer.getTime() + memCpyD2HTimer.getTime()) << std::endl;
